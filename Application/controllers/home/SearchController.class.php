...
class SearchController extends BaseController 
{
	...
	public function searchAction(){
		...
		$priceSection = 7; // 要分的段数
		// 取出扩展分类下的商品的ID并转化成字符串：1,2,3,4,5
		$sql = "select GROUP_CONCAT(DISTINCT goods_id) goods_id from php34_goods_cat where cat_id =$catId";
		$extGoodsId = $goodsModel->db->getRow($sql);
		if($extGoodsId['goods_id'])
			$extGoodsId = " OR id IN({$extGoodsId['goods_id']})";
		else 
			$extGoodsId = '';
		// 主分类和扩展分类下的商品都搜索出来
		// 算法：取出这个分类下商品的最低价和最高价
		$sql = "select MIN(shop_price) minprice,MAX(shop_price) maxprice from php34_goods where cat_id=$catId $extGoodsId";
		$price = $goodsModel->db->getRow($sql);
		// 最低价和最高价分七段
		$_price = array();
		// 计算每个段应该价格区间
		$sprice = ($price['maxprice'] - $price['minprice']) / $priceSection;
		...
		for($i=0; $i<$priceSection; $i++)
		{
				...
				$end = (floor(($firstPrice+$sprice)/10)*10-1);
				// 先判断这个分类下在这个价格段中是否有商品
				$sql = "where shop_price between $start and $end and (cat_id=$catId $extGoodsId) and is_on_sale=1 and is_delete=0";
				$goodsCount = $goodsModel->total($sql);
				$firstPrice+=$sprice;
				if($goodsCount==0)
					continue;
				$_price[] = $start . '-'. $end;
		...
		$sai = $catModel->getOneCat($catId);
		// 根据筛选属性的ID取出这些属性的名称以及每个属性所拥有的值
		$attrModel = new AttributeModel();
		$sql = "select id,attr_name from php34_attribute where id in (".$sai['search_attr_id'].")";
		$attrData = $attrModel->Mquery($sql);
		// 循环所有的筛选属性，取出这些属性中有商品的值
		foreach ($attrData as $k => $v)
		{
			// 找出这个属性有商品的值  --> 从商品属性表(一件商品所拥有的属性以及值）
			$sql = "select DISTINCT attr_value from php34_goods_attr where attr_id=".$v['id'];
			$attrValues = $attrModel->Mquery($sql);
			// 如果这个属性下没有商品,那么就删除这个属性
			if(!$attrValues)
				unset($attrData[$k]);
			else 
		...
		// 取出商品
		$sql = "select * from php34_goods where cat_id=$catId $extGoodsId";
		$goods['data'] = $goodsModel->Mquery($sql);
		
		$this->assign('goods',$goods);
		$this->assign('attrData',$attrData);
		$this->assign('price',$_price);
		// 设置页面的信息
		$this->setPageInfo('搜索页', '搜索页', '搜索页', 0, array('list','common'), array('list'));
		// 显示页面
		$this->display();
