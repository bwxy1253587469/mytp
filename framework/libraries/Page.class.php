...
class Page{
	...
	//构造方法
	/**
	 *@param script string 超链接地址
	 */
	function __construct($total,$pagesize,$current,$script = ''){
		$this->total = $total;
		$this->pagesize = $pagesize;
		$this->current = $current;
		$this->pagenum = ceil( $total / $pagesize );
		//index.php?p=admin&c=brand&a=index&page=2
		//new分页类 new Page(10,3,2,'index.php?p=admin&c=brand&a=index&page=2')
		$script = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
		if(strpos($script,'?'))
			$this->url = "http://".preg_replace('#page=[0-9]?#','',$script)."&page="; //由于上页，下页，首页，末页的page是不同的，不能指定
		else
			$this->url = "http://".$script."?page=";
...
/*
		require_once LIB_PATH."page.class.php";//导入page类
		isset($_GET['page'])?$p=$_GET['page']:$p=1;//设置当前页数
		$count = $this->total($where);//总的记录条数
		$per = 5;						//每页记录条数
		$page = new Page($count,$per,$p);
		//echo "     ".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']."<br>";
		$where .=" limit ".($p-1)*$per.",".$per;//组织查询语句
		$goods['data'] = $this->selectAll($where);
